...
import { listMatchings, listMentees, listMentors } from "@/features/signup/lib/repository";
...
export default async function AdminPage() {
  ...
  const matchRows: MatchRow[] = matchings.map((m) => ({
    ...
    areaScore: round1(m.breakdown?.areaAndPersona),
    majorScore: round1(m.breakdown?.majorAndCareer),
    basicScore: round1(m.breakdown?.basics),
    contact: m.mentorContact,
    status: m.status,
  }));

  return <AdminDashboard mentees={mentees} mentors={mentors} matchRows={matchRows} />;
}
