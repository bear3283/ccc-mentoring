...
**출신 고등학교 · 추천인 · MBTI는 수집하지만 점수에 넣지 않습니다.**

- **고등학교 · 추천인** — 사실 확인할 방법이 없어, 적어 낸 값을 그대로 믿고 점수를 주면
  표기 차이(`한빛고` vs `한빛고등학교`)나 오타만으로 순위가 흔들립니다
- **MBTI** — 성향은 이미 성경 인물이 2순위(30%)를 맡고 있습니다. MBTI까지 점수에 넣으면
  배분을 다시 나눠야 해서, 지금은 카드 해시태그(`#ENFP`)와 운영자 표에만 보여줍니다
