...
| --- | --- | --- |
| `/` | 참가자 | 역할 선택 (멘티 / 멘토) |
| `/onboarding/mentee` | 고3 학생 | 멘티 신청 7스텝 |
| `/onboarding/mentor` | CCC 재학생 | 멘토 신청 7스텝 |
...
첫 화면에서 역할을 고릅니다. "멘티로 시작하기"를 누르면 7스텝이 시작됩니다.
...
| 2 | ![](screenshots/01-멘티-고3학생/02-지망캠퍼스-1~3지망.png) | **지망 캠퍼스** — 누른 순서가 곧 지망 순위(1·2·3). 매칭 가중치 50%로 가장 무겁습니다 |
| 3 | ![](screenshots/01-멘티-고3학생/03-관심분야-3열그리드.png) | **관심 분야** — 3열 격자, 최대 3개. 고른 항목의 설명만 아래에 모아 보여줍니다 |
| 4 | ![](screenshots/01-멘티-고3학생/04-희망학과와진로.png) | **희망 학과·진로** — 각각 최대 3개. `0/3` 카운터로 남은 개수를 표시합니다 |
| 5 | ![](screenshots/01-멘티-고3학생/05-프로필-이름연락처.png) | **프로필** — 이름·성별·연락처는 필수, 고등학교·추천인은 선택 |
| 6 | ![](screenshots/01-멘티-고3학생/06-가능시간대-밤은온라인.png) | **가능 시간대** — 평일/주말 × 5구간. **밤(21-24시)은 온라인** 뱃지로 표시 |
| 7 | ![](screenshots/01-멘티-고3학생/07-프로필사진-건너뛰기가능.png) | **프로필 사진** — 선택. 없으면 성향 이모지로 대체됩니다 |
| 8 | ![](screenshots/01-멘티-고3학생/08-신청완료-참여코드발급.png) | **신청 완료** — 참여코드 발급. 코드를 누르면 복사됩니다 |
| 9 | ![](screenshots/01-멘티-고3학생/09-매칭결과-멘토카드3장.png) | **매칭 결과** — 적합도 순 상위 3명의 프로필 카드 |
...
| 순위 | 항목 | 배점 | 계산 방식 |
| --- | --- | --- | --- |
| 1 | 지망 캠퍼스 | **50** | 1지망 100% · 2지망 80% · 3지망 60% |
| 2 | 관심 영역 + 성향 | **30** | 영역 겹침 비율 60% + 성경 인물 궁합 40% |
| 3 | 학과 · 진로 | **15** | 교집합 1개 이상이면 0.5, 전부 겹치면 1.0 |
| 4 | 기본 | **5** | 시간대 겹침 정도 70% + 동성 여부 30% |
...
**출신 고등학교와 추천인은 수집하지만 점수에 넣지 않습니다.** 지금은 사실 확인할
방법이 없어, 적어 낸 값을 그대로 믿고 점수를 주면 표기 차이(`한빛고` vs `한빛고등학교`)나
오타만으로 순위가 흔들립니다. 운영자 표에서 참고용으로만 봅니다.
...
점수에 새어 들어갔는지(소스 직접 검사), 참여코드 유일성, 점수 범위, 필터 동작,
정렬 순서 등 **21개 항목**을 확인합니다.
...
├─ features/
│  ├─ onboarding/
│  │  ├─ components/steps/       스텝 하나 = 파일 하나 (7개)
...
│
└─ shared/
   ├─ constants/{domain,persona,role}   온보딩과 매칭이 함께 참조
